...
test("loads and validates a manifest", () => {
  ...
  assert.deepEqual(Object.keys(loaded.manifest.modules).sort(), ["", "images"]);
  assert.equal(loaded.dir, projectDir);
});

test("resolves the default module (module: null)", () => {
  const loaded = loadManifest(project);
  const resolved = resolveModule(loaded, null);
  assert.equal(resolved.language, "ts");
  assert.equal(resolved.functions, resolve(projectDir, "src", "tasks.ts"));
});

test("resolves a module whose functions live outside the project dir", () => {
  const loaded = loadManifest(project);
  const resolved = resolveModule(loaded, "images");
  assert.equal(resolved.language, "py");
  assert.equal(resolved.functions, resolve(projectDir, "..", "shared", "img.py"));
